...
//Device Check
if((winH / winW) < 0.5) $('html').addClass('homebar');
        ...
        if($gnb.length){
            ...
            if(isNumeric(gnbIdx)){//숫자로 입력
                ...
                if(sub == '' || !sub){
                    ...
                    //2depth activation
                    if(!depth2) depth2 = 0;
                    $subMenu.find('.swiper-slide').eq(depth2).addClass('is-active').siblings('li').removeClass('is-active');
                    $('.app-lnb').addClass('app-sub-active');
                    subMenuSwiper.slideTo(depth2)
...
function wishCarStrategyInit() {
    ...
    var $notEnough =  $section.parents(".popup").find(".not.enough");
    var $enough = $section.parents(".popup").find(".enough").not(".not");
...
//직원조회(임시)
$('.popup[data-layered-name=popupEmployeeSearch] .branch-search-list').on('click','button',function(){
    $('.popup[data-layered-name=popupEmployeeSearch]').find('.popup_contents').removeClass('is-branch-search').addClass('is-employee-search');
});
...
// SVG Gauge Functions
function drawInsuranceGauge(selector, current, grade) {
    var obj = $(selector);
    obj.parent().prev().addClass("g"+grade);
    var colors = ['','#4dabff','#ffd338','#d672f6']; // 1: 충분, 2: 유의, 3: 부족
    setGauge(selector, current, 100, colors[grade]);
}
